...
def _heuristic_score(global_index: int, item: dict) -> tuple[float, float, bool, str]:
    ...
    relevance = min(0.95, 0.62 + min(text_len, 240) / 800)
    quality = max(0.55, 0.88 - (global_index / 1200.0) +
                  (0.02 if text_len > 80 else -0.01))
    keep = relevance >= 0.65 and quality >= 0.60
    reason = "heuristic: title/snippet length and position-based quality"
